...
You can use subagents for tasks, but collecting the full output of the run will cause context poisoning. Query them to return just the answer.

## Task Models
Choose the model (haiku, sonnet, opus) that is enough for the subtask. Criticism, writing and discussion tasks must use opus. File management, bibliography and other tasks can be good on sonnet (the standard programming model), and low risk tasks (never involving the file system as sandbox is not granted) can run haiku.
...
1. Immediately pick the next productive task from cycles/index.md
